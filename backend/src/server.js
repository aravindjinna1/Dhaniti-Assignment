...
async function startServer() {
  ...
  if (process.env.DATABASE_URL) {
    ...
    if (isDbConnected) {
      ...
      console.warn('  PostgreSQL connection failed. Using verified in-memory dataset.');
      console.warn('   To seed PostgreSQL, run: npm run db:seed');
    }
  } else {
    console.log('  No DATABASE_URL configured. Running with in-memory cleaned CSV data.');
  }
